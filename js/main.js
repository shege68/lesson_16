...
function wordsList(str, subStr) {
  let strFix = str.replaceAll(/[\\.,!\\?;:"]/g, '');
  strFix = strFix.toLowerCase();

  let arr = strFix.split(' ');

  let arrFix = arr.filter(arrItem => {
    if (arrItem.indexOf(subStr) > -1) return true;
  });
...
let car = new Car(1600, 'Aveo', 'Chevrolet', 2008);
let car2 = new Car(2500, 'Camry', 'Toyota', 2020);
